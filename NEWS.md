...
* Replace the original `rmarkdown::render_supporting_files()` to a new
  `render_supporting_files_2()`, which copies missing and modified files
  from the source directory to the destination. This helps keep the 
  destination tree updated when I rebuild things like `reveal.js` themes
  and re-render documents.
...
* Major internal refactoring to do better at merging user options
  and default options for reveal.js configuration variables.
